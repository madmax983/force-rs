...
| [009](009-decouple-storage.md) | Decouple Storage from Core | Proposed | 2026-02-09 |

### Future ADRs

Planned ADRs for upcoming decisions:
- **ADR-010**: HTTP Client and Middleware Architecture
- **ADR-011**: Rate Limiting and Retry Strategy
- **ADR-012**: Token Storage and Caching
- **ADR-013**: SOQL Query Builder Design
- **ADR-014**: Pub/Sub gRPC Integration
- **ADR-015**: Testing Strategy and Mock Framework
- **ADR-016**: Logging and Observability
- **ADR-017**: Connection Pooling Configuration
...
graph TD
    ...
    ADR001 --> ADR009[ADR-009: Decouple Storage]
    ...
    style ADR008 fill:#20c997
    style ADR009 fill:#ff922b
```
...
- **Rationale**: Compile-time prevention of invalid state transitions.
- **Impact**: Compile-time safety eliminates entire class of runtime errors.

### ADR-009: Decouple Storage from Core
- **Decision**: Move persistence logic to a dedicated `storage` module.
- **Rationale**: Breaks the circular dependency cycle.
- **Impact**: Build times improve, but FFI complexity increases.
