...
**Gap Analysis:**
The `force` crate currently supports modern API surfaces (REST, Bulk V2, Composite, GraphQL) but has no XML/SOAP serialization or deserialization capabilities. Rust has a different ecosystem for XML compared to JSON . We lack a `soap()` handler and the necessary tooling to generate Rust types from the Enterprise WSDL or interact with the Partner WSDL dynamically.
...
- Must support passing a session ID obtained via standard authentication (OAuth) to the SOAP header, avoiding the need to use the legacy SOAP `login()` if modern auth is available.
- Success = A developer can execute a basic SOAP `query` or `create` using models that map to the XML schema.

🚫 **Out of Scope:**
- Automatic runtime WSDL parsing (we assume users will use pre-generated code or standard dynamic models).
