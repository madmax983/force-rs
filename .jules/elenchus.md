...
**[Elenchus: api::soap::types Test Quality Audit]**
**Module:** `force::api::soap::types`
**Severity:** 🔴 Critical
**Finding:** The `SObject` builder and accessor methods lacked dedicated unit tests. Tests covering serialization indirectly exercised these methods, but `cargo mutants` revealed that they did not strongly assert the resulting state, allowing mutations in `with_field`, `with_null_field`, `set_field`, `get`, and `id` to survive.
**Evidence:** 10 surviving mutants around `SObject` builder methods returning `Default::default()`, `set_field` being replaced with `()`, `get` returning `None`, `Some("")`, or swapping `==` with `!=`, and `id` returning `None` or `Some("")`.
**Recommendation:** Sentry must add direct unit tests for `SObject::new`, `with_field`, `with_null_field`, `set_field`, `get`, and `id` to explicitly assert the exact state of the `fields` and `fields_to_null` collections, ensuring strong independent validation.
