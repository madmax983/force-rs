...
**Module:** crates/force/src/api/rest_operation.rs
**Severity:** 🔴 Critical
**Finding:** Missing mutation test coverage in upsert_with_retry_class. The response limit calculation in upsert lacked explicit bound verification allowing false limits like 100KB to slip through.
**Evidence:** Surviving mutants around 100 * 1024 * 1024 limit calculation in upsert paths (* -> +).
**Recommendation:** Extract the limit to a constant and add a test asserting the exact constant mathematical bounds.
