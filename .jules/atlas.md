...
**Unified error types in force workspace crates**
**Tangle:** Result types were verbosely declared in every file using `Result<T, ErrorType>`, causing repetitive code and inconsistent use of standard Result types across the `force` and `force-sync` crates.
**Blueprint:** Replaced explicit `Result<T, ErrorType>` with `crate::error::Result<T>` in all workspace crates (`force` and `force-sync`), adopting the idiomatic Rust standard of a single `Result` alias in the crate's `error.rs` file. This creates cleaner imports and unified error handling boundaries.
