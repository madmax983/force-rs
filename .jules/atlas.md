...
**[The Facade: Fixing the Leak in SqlExporter]**
**Tangle:** The `experimental::sql_exporter` module leaked its internal structure by being declared as `pub mod`, forcing users to import from a nested path.
**Blueprint:** Changed visibility to `pub(crate) mod` and explicitly re-exported `SqlExporter` using `pub use` at the module root, enforcing the Facade pattern.
