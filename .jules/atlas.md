...
**[Fixing The Leak and Missing Feature Gate]
**Tangle:** The experimental Query Plan API (`explain`) was compiled unconditionally despite being intended for the `nova` feature, and its response types (`ExplainResponse`, `QueryPlan`, `PlanNote`) were hidden in a `pub(crate)` module but returned by a public method, creating a leaky abstraction and orphaned types.
**Blueprint:** Added the `nova` feature to `Cargo.toml`. Applied `#[cfg(feature = "nova")]` to the `explain` module, its exports, and the `RestHandler::explain` function. Used `pub use explain::{...}` at the module root (`api::rest::mod`) to correctly expose the types and enforce high cohesion.
