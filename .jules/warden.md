...
**2026-04-03 - [DoS via Unbounded Allocation in Bulk API Query]
**Threat:** The `deserialize_csv` method in `crates/force/src/api/bulk/query.rs` used `response.bytes().await` to read the entire CSV payload into memory. A maliciously large response from the API server (Salesforce bulk limits up to 1GB, but theoretically unbounded if abused) could exhaust all available memory and lead to an OOM panic (Denial of Service).
**Defense:** Replaced `response.bytes().await` with `crate::http::error::read_capped_body_bytes(response, 1024 * 1024 * 1024).await?` to enforce a strict 1GB maximum size limit, effectively capping the allocation and propagating an InvalidInput error if the limit is exceeded.
