...
**2024-05-25 - [Bypass of 1MB error body cap via large single chunk]**
**Threat:** While the error response readers in `http/error.rs`, `auth/client_credentials.rs`, and `auth/jwt_bearer.rs` enforced a 1MB limit by checking `bytes.len() > 1024 * 1024`, they first appended the entire chunk (`bytes.extend_from_slice(&chunk_bytes)`). A malicious server returning a single massive chunk could bypass the limit temporarily, causing unbounded memory allocation before the truncation logic could execute, leading to an OOM DoS.
**Defense:** Extracted the loop into a single `read_capped_body` function. Calculated `remaining` capacity correctly and explicitly constrained the `chunk_bytes` to `std::cmp::min(remaining, chunk_bytes.len())` before extending the vector, thereby strictly avoiding any allocation exceeding 1MB.
