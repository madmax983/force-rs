...
## 2024-06-26 - [DoS via Unbounded Memory Allocation in ClientCredentials]
**Threat:** The `ClientCredentials` authenticator's constructor `new` initialized the `reqwest::Client` with a hardcoded `.unwrap_or_else(|e| panic!(...))`. While TLS/backend initialization errors are rare on modern platforms, failing to instantiate the client causes a process-level panic, representing a potential Denial of Service vector if an attacker can manipulate environment/TLS configuration or if it fails unexpectedly.
**Defense:** Replaced the panicking `.unwrap_or_else` call with robust error propagation `Result<Self>` in `ClientCredentials::new`, `new_production`, and `new_sandbox`, returning a strictly typed `ForceError::Authentication(AuthenticationError::InvalidJwtConfig(...))` (repurposed or matched to the appropriate HTTP init error) so that the calling application can handle the failure gracefully. Updated all tests and examples to handle the `Result`.
