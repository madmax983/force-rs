...
## 2026-03-05 - [DoS via Unbounded Counter Integer Overflow]
**Threat:** The `http` and `bulk` modules incremented polling and retry attempts (e.g. `attempt += 1`, `retry_attempt += 1`) without bounds checking. An attacker who could theoretically keep triggering timeouts or transient errors indefinitely (or a misconfiguration setting `max_attempts` to `u32::MAX`) would cause these counters to eventually overflow `u32`, panicking the application and causing a Denial of Service.
**Defense:** Replaced `+= 1` with `saturating_add(1)` on retry and poll attempt counters across `crates/force/src/http/executor.rs`, `crates/force/src/api/bulk/ingest.rs`, `crates/force/src/api/bulk/query.rs`, `crates/force/src/api/bulk/smart_ingest.rs`, and `crates/force/src/api/bulk/policy.rs`. This prevents overflow panics and caps behavior.
