...
## 2026-04-02 - [DoS / Deserialization Bomb in Userinfo Response Parsing]
**Threat:** The `fetch_tenant_id` function read the userinfo response chunk by chunk with a 10MB limit. However, when the limit was exceeded, it silently truncated the payload and proceeded to parse the corrupted JSON via `serde_json::from_slice`. An attacker could exploit this by sending a malformed or extremely large payload designed to bypass limit logic or cause extensive memory/CPU consumption through recursive parsing of the truncated JSON payload.
**Defense:** Replaced the silent buffer truncation logic with an explicit, fail-fast return (`Err(PubSubError::Config("userinfo response exceeded size limit".to_string()))`) to terminate processing immediately when the payload exceeds the allocated limit, thereby preventing resource exhaustion and avoiding downstream errors from parsing malformed, truncated JSON.
...
**2026-04-10 - [DoS / Deserialization Bomb in files.rs read_capped_body_bytes]
**Threat:** The `read_capped_body_bytes` function inside `crates/force/src/api/files.rs` was designed to prevent memory exhaustion by capping the response body size. However, it silently truncated the payload when the limit was exceeded and returned the partial bytes. In cases where the response was expected to be parsed as JSON, passing a truncated payload to `serde_json::from_slice` could lead to deserialization errors, unbounded parsing on corrupted payloads (deserialization bombs), and masking of the actual error (resource exhaustion).
**Defense:** Refactored `read_capped_body_bytes` to return `Result<bytes::Bytes, ForceError>`. It now explicitly fails fast and returns `ForceError::InvalidInput("response body exceeded size limit")` when the size limit is exceeded.
