...
**Threat:** The `upload` and `link_to_record` methods in the Files API used `read_capped_body` to read the response into memory and then parsed it into an unbounded `serde_json::Value` DOM structure. Because the response can be up to 100MB, parsing it into a generic DOM can consume over a gigabyte of memory, leaving it highly vulnerable to memory exhaustion DoS attacks via deserialization bombs.
**Defense:** Replaced the intermediate `String` allocation from `read_capped_body` with `read_capped_body_bytes` to stream bytes directly into a vector. Then, strictly parsed the bytes into a limited, strongly-typed struct (`FilesUploadResponse` / `FilesLinkResponse`) using `serde_json::from_slice`, eliminating the massive DOM allocation entirely.
