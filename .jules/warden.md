...
**2024-06-25 - [Memory Exhaustion DoS in Error Parsing]**\n**Threat:** Deserialization bombs and DoS. Maliciously large error payloads could be sent by compromised or malicious endpoints to consume all server memory. The chunk reading loop would silently truncate payload sizes exceeding  allowing partially invalid JSON to proceed to the  parser.\n**Defense:** Changed  and  to immediately return  with an error when . Fallback cases gracefully handle this result using  to preserve status-code parsing, preventing deserialization errors on half-read JSON and mitigating resource exhaustion.

**2024-06-25 - [Memory Exhaustion DoS in Error Parsing]**
**Threat:** Deserialization bombs and DoS. Maliciously large error payloads could be sent by compromised or malicious endpoints to consume all server memory. The chunk reading loop would silently truncate payload sizes exceeding `limit_bytes` allowing partially invalid JSON to proceed to the `serde_json::from_slice` parser.
**Defense:** Changed `read_capped_body` and `read_capped_body_bytes` to immediately return `Result<Vec<u8>, ForceError>` with an error when `chunk_bytes.len() > remaining`. Fallback cases gracefully handle this result using `.unwrap_or_default()` to preserve status-code parsing, preventing deserialization errors on half-read JSON and mitigating resource exhaustion.
