...
## 2026-03-07 - [SSRF/Credential Injection via Relative URL]
**Threat:** The `resolve_next_records_url` method validated absolute `nextRecordsUrl` strings correctly but fell back to plain string concatenation for relative paths (e.g. `format!("{}{}", instance_url, next_records_url)`). An attacker could bypass absolute URL checks and exploit string concatenation by supplying a relative URL starting with `@` (e.g., `@attacker.com/capture`). This forces the `instance_url` to be parsed as basic auth credentials while directing the network request (and the sensitive Bearer token) to the attacker's host.
**Defense:** Added a check requiring that all relative `next_records_url` payloads begin with a forward slash (`/`).
