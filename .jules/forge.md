...
**Flattening retry_loop in HttpExecutor**
**Learning:** `retry_loop` had deep nesting where `if status == StatusCode::UNAUTHORIZED` contained an inner `if !refreshed`, causing unnecessary indentation and reducing readability.
**Action:** Flattened by combining conditions into an early return guard (`if status == StatusCode::UNAUTHORIZED && !refreshed`). This reduces cognitive load while maintaining the exact same retry behavior.
