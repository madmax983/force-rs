...
**[Extract Retry Loop Evaluation Helper]**
**Learning:** Having duplicated nested match/if statements ("Pyramid of Doom") across similar loop functions (`retry_loop` and `retry_loop_factory`) makes the execution control flow harder to understand and maintain.
**Action:** Encapsulate execution response evaluation into an enum like `RetryAction` and a helper method. This flattens the loop control flow and clearly documents transitions like `Retry` or `RefreshAndRetry`.
