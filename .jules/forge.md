...
\n**[Extract God Functions - match statements]**\n**Learning:** Massive `match` statements on complex enums inside loops create 'God Functions' that are hard to read and maintain.\n**Action:** Extract large `match` blocks into dedicated, well-named helper functions (e.g., `apply_field_type_schema`) to flatten the structure and reduce cognitive load.
