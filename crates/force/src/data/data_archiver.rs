...
impl<'a, A: Authenticator> DataArchiver<'a, A> {
        ...
        T: DeserializeOwned + Serialize + Unpin,
    {
        if path.as_ref().components().any(|c| matches!(c, std::path::Component::ParentDir)) {
            return Err(ForceError::InvalidInput("Path traversal detected".to_string()));
        }
...
mod tests {
    ...
    async fn test_export_path_traversal() {
        ...
        let traversal_path = "../tmp/export.jsonl";
        ...
        let result_masked = archiver
            .export_masked_to_jsonl("Account", soql, traversal_path)
            .await;

        assert!(matches!(result_masked, Err(ForceError::InvalidInput(_))));
    }
}
