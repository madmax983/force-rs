...
impl<'a, A: Authenticator> DataArchiver<'a, A> {
    ...
    pub async fn export_to_csv(&self, soql: &str, path: impl AsRef<Path>) -> Result<usize> {
        ...
        while let Some(record) = stream.next().await? {
            ...
            for header in &headers {
                if header == "Id" {
                    if let Some(val) = record.fields.get("Id") {
                ...
                } else if let Some(val) = record.fields.get(header) {
                    let val_str = match val {
                        serde_json::Value::String(s) => s.clone(),
                        serde_json::Value::Null => String::new(),
                        other => other.to_string(),
        ...
        wtr.flush()
            .map_err(|e| ForceError::from(SerializationError::from(csv::Error::from(e))))?;
        Ok(count)
    }
    ...
    /// # Arguments
    ///
    /// * `sobject_name` - The API name of the SObject being queried.
    /// * `soql` - The SOQL query string.
    /// * `path` - The file path to write the CSV output.
    ///
    /// # Returns
...
mod tests {
    ...
    async fn test_export_to_csv() {
        ...
        // CSV should have a header line + 2 data lines, depending on dynamic SObject struct.
