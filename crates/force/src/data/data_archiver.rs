...
mod tests {
    ...
    #[tokio::test]
    async fn test_export_masked_to_jsonl_null_fields() {
        let mock_server = MockServer::start().await;

        let describe_json_result: std::result::Result<serde_json::Value, _> = serde_json::from_str(
            r#"{
            "name": "Contact",
            "label": "Contact",
            "custom": false,
        ...
        let Ok(describe_json) = describe_json_result else {
        ...
        let client_result = ForceClientBuilder::new().authenticate(auth).build().await;
        ...
        let file_path =
            env::temp_dir().join(format!("export_masked_null_{}.jsonl", std::process::id()));

        let soql = "SELECT Id, Name, Email FROM Contact";
        let count_result = archiver
            .export_masked_to_jsonl("Contact", soql, &file_path)
            .await;
        let Ok(count) = count_result else {
            panic!("Export failed")
        };

        assert_eq!(count, 1);

        let contents_result = std::fs::read_to_string(&file_path);
        ...
        let record_result: std::result::Result<serde_json::Value, _> =
            serde_json::from_str(lines[0]);
        let Ok(record) = record_result else {
            panic!("Failed to parse record")
        };

        assert_eq!(record["Name"], "John Doe");
        assert!(record["Email"].is_null()); // Null should remain null!
