...
mod tests {
    ...
    fn test_query_result_invalid_state_not_done_without_next_url() {
        ...
        assert!(!result.is_done());
        // Fix: even though done=false, without a next url, we don't have more
        assert!(!result.has_more());
    ...
    mod proptests {
        ...
        proptest! {
            // Property 1: has_more() implies not done AND has a url
            #[test]
            fn prop_has_more_implies_not_done_and_has_url(result in arbitrary_query_result()) {
