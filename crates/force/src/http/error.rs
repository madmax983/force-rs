...
fn try_parse_salesforce_error(body: &str) -> Option<String> {
    let errors = serde_json::from_str::<Vec<SalesforceError>>(body).ok()?;
...
pub fn parse_api_error(status_code: u16, body: &str) -> HttpError {
    HttpError::StatusError {
        status_code,
        message: try_parse_salesforce_error(body).unwrap_or_else(|| body.to_string()),
    }
}
