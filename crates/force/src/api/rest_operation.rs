...
async fn upsert_with_retry_class_impl<A: Authenticator>(
    ...
    // ⚡ Bolt: Replace `format!` with `String::with_capacity` and sequential `.push_str()` calls
    // to eliminate intermediate formatting overhead and string allocations in this hot path.
    let encoded_value = utf8_percent_encode(external_id_value, UPSERT_ENCODE_SET);

    // Calculate required capacity
    let encoded_len: usize = encoded_value.clone().map(str::len).sum();
