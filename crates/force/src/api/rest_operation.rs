...
async fn upsert_with_retry_class_impl<A: Authenticator>(
    ...
    // ⚡ Bolt: Eliminate `format!` macro overhead in hot path URL construction.
    // Pre-calculate exact capacity and sequentially push strings to avoid multiple heap reallocations.
    ...
    let mut api_path = String::with_capacity(
        prefix_len + 11 + sobject.len() + external_id_field.len() + encoded_len,
    );
