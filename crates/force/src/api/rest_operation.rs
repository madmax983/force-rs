...
async fn upsert_with_retry_class_impl<A: Authenticator>(
    ...
    // ⚡ Bolt: Calculate exact capacity and use `.push_str()` instead of `format!` to eliminate intermediate allocations.
    let encoded_value = utf8_percent_encode(external_id_value, UPSERT_ENCODE_SET);
    let encoded_len = encoded_value.clone().map(str::len).sum::<usize>();
    ...
    let mut api_path = String::with_capacity(
        prefix_len + 10 + sobject.len() + 1 + external_id_field.len() + 1 + encoded_len,
    );
