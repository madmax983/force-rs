...
async fn upsert_with_retry_class_impl<A: Authenticator>(
    ...
    // ⚡ Bolt: Avoid `format!` string allocations by streaming the percent-encoded
    // parts directly into a pre-allocated buffer before resolving the URL.
    let mut api_path = String::with_capacity(
        if api_path_prefix.is_empty() { 0 } else { api_path_prefix.len() + 1 }
            + 11 // "sobjects/" + "/" + "/"
            + sobject.len()
            + external_id_field.len()
            + external_id_value.len() * 3, // Safe upper bound for percent encoding
    );
