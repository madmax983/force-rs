...
async fn upsert_with_retry_class_impl<A: Authenticator>(
    ...
    // ⚡ Bolt: Avoid `format!` overhead when formatting `PercentEncode` by pre-allocating
    // capacity and appending chunks directly. `format!` dynamically reallocates for Display traits of unknown length.
    let mut capacity = 11 + sobject.len() + external_id_field.len() + external_id_value.len();
    if !api_path_prefix.is_empty() {
        capacity += api_path_prefix.len() + 1;
    }
    capacity += 16; // Buffer for potential percent encoding expansion

    let mut api_path = String::with_capacity(capacity);
    if !api_path_prefix.is_empty() {
    ...
    api_path.push('/');
    for chunk in utf8_percent_encode(external_id_value, UPSERT_ENCODE_SET) {
