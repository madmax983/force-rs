...
async fn upsert_with_retry_class_impl<A: Authenticator>(
    ...
    // ⚡ Bolt: Pre-allocate the required capacity and append percent-encoded chunks
    // directly in a loop to avoid double-iterating the encoder for size hints
    // or relying on `format!`'s dynamic reallocation.
    let encoded_value = utf8_percent_encode(external_id_value, UPSERT_ENCODE_SET);

    let mut api_path = String::with_capacity(
        api_path_prefix.len()
            + usize::from(!api_path_prefix.is_empty())
            + 11 // "sobjects/" + "/" + "/"
            + sobject.len()
            + external_id_field.len()
            + external_id_value.len(), // lower bound for encoded length
    );
