...
impl<A: crate::auth::Authenticator> crate::api::ui::UiHandler<A> {
    ...
    pub async fn record_ui(
        ...
        // ⚡ Bolt: Construct comma-separated strings dynamically to avoid intermediate Vec allocation from `.collect::<Vec<_>>().join()`
        let lt_str = layout_types.map(|lts| {
            let mut s = String::with_capacity(lts.len() * 15);
