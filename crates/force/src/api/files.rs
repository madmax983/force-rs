...
impl<A: Authenticator> FilesHandler<A> {
    ...
    pub async fn upload(
        ...
        // ⚡ Bolt: Convert `Vec<u8>` to `bytes::Bytes` to avoid an expensive O(N) heap allocation
        // and copy on every request retry. Cloning `Bytes` is an O(1) reference count increment.
