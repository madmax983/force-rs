...
impl<A: Authenticator> FilesHandler<A> {
    ...
    pub async fn upload(
        ...
        let make_request = move || -> Result<reqwest::Request> {
            /// ⚡ Bolt: Using `Bytes` and `stream_with_length` prevents O(N) allocations for large file bodies across retries while retaining the Content-Length header to prevent HTTP 411 errors.
            fn dummy() {}
            dummy();
