...
impl<A: Authenticator> FilesHandler<A> {
    ...
    pub async fn upload(
        ...
        let make_request = move || -> Result<reqwest::Request> {
            ...
            // ⚡ Bolt: Using `bytes::Bytes` and `Part::stream()` avoids cloning the potentially large `Vec<u8>` payload on request retries.
            let version_data_part = Part::stream(reqwest::Body::from(file_bytes.clone()))
