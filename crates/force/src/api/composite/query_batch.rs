...
impl<A: Authenticator> QueryBatch<A> {
    ...
    /// # Arguments
    ///
    /// * `session` - The Force client inner session.
        ...
        F: Fn(T) -> Option<BatchOp>,
    {
        ...
        let rest_handler = crate::api::rest::RestHandler::new(self.session.clone());
        let mut result = rest_handler.query::<T>(&self.query).await?;
        ...
        loop {
            ...
            if let Some(next_url) = result.next_records_url {
                result = rest_handler.query_more(&next_url).await?;
            } else {
                break;
            }
        }
    ...
    async fn flush_batch(&self, ops: &mut Vec<BatchOp>, stats: &mut BatchStats) -> Result<()> {
        ...
        let composite_handler = crate::api::composite::CompositeHandler::new(self.session.clone());
        let mut batch = composite_handler.batch().halt_on_error(self.halt_on_error);
