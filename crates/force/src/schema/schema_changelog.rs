...
mod tests {
    ...
    fn mock_field(name: &str, field_type: &str) -> serde_json::Value {
        let ft = match field_type {
            ...
            "textarea" => crate::types::describe::FieldType::Textarea,
            "int" => crate::types::describe::FieldType::Int,
            _ => crate::types::describe::FieldType::String,
        };
        serde_json::to_value(crate::test_support::MockFieldDescribeBuilder::new(name, ft).build())
            .must()
    }
    ...
    fn test_schema_changelog_generator_with_changes() {
        ...
        assert!(md.contains("| NewField | `NewField` | Int |"));

        assert!(md.contains("## Removed Fields"));
        assert!(md.contains("| OldField | `OldField` | String |"));
