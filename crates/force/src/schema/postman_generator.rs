...
pub fn generate_postman_collection(describe: &SObjectDescribe) -> Value {
    ...
    json!({
        ...
        "item": [
            create_request(name, &label, &create_body_str),
            read_request(name, &label),
            update_request(name, &label, &update_body_str),
            delete_request(name, &label),
        ]
    })
}

#[cfg(feature = "schema")]
fn create_request(name: &str, label: &str, create_body_str: &str) -> Value {
    json!({
...
#[cfg(feature = "schema")]
fn read_request(name: &str, label: &str) -> Value {
    json!({
...
#[cfg(feature = "schema")]
fn update_request(name: &str, label: &str, update_body_str: &str) -> Value {
    json!({
...
#[cfg(feature = "schema")]
fn delete_request(name: &str, label: &str) -> Value {
    json!({
