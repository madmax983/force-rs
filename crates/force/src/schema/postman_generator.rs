...
pub fn generate_postman_collection(describe: &SObjectDescribe) -> Value {
    ...
    // ⚡ Bolt: Using a borrowed string slice instead of an owned `String` avoids an unnecessary heap allocation, as the label is only used for interpolation into `format!` macros below.
