...
pub fn write_go_struct(out: &mut String, describe: &SObjectDescribe) {
    ...
    let _ = writeln!(out, "// Label: {}", describe.label);
    ...
    for field in fields {
        ...
        let (type_prefix, omitempty) = if field.nillable {
            ("*", ",omitempty")
        } else {
            ("", "")
        };
        ...
        let _ = writeln!(
            out,
            "\t{} {}{} `json:\"{}{}\"`",
            field.name, type_prefix, go_type, field.name, omitempty
        );
    }
...
fn map_type(ft: &FieldType) -> &'static str {
    match ft {
        ...
        // Typically serialized as ISO 8601 strings, IDs, strings, etc.
        _ => "string",
    }
}
