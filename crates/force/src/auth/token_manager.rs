...
impl<A: Authenticator> TokenManager<A> {
    ...
    pub async fn force_refresh(&self) -> Result<AccessToken> {
        ...
        // Double check: Did another thread already refresh the token while we were waiting?
        {
            let state = self.state.read().await;
            if let Some(token) = &state.token {
                if let Some(current) = &current_arc {
                    // If the Arc pointer is different, another thread has replaced the token
                    if !std::sync::Arc::ptr_eq(token, current) {
                        return Ok((*token.clone()).clone());
                    }
                } else {
                    // We didn't have a token before, but now we do. Someone else refreshed!
                    return Ok((*token.clone()).clone());
                }
            }
        }
...
mod tests {
    ...
    async fn test_token_manager_equality_overwrites() {
        ...
        // Since `old.issued_at > new.issued_at` is false, it WILL overwrite
        ...
        // Now let's test equality overwrite for hard expiration
        ...
        // Now let's test equality overwrite for soft expiration
