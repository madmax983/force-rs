...
mod example {
    ...
    pub async fn main() -> anyhow::Result<()> {
        ...
        // 1. Query some records to delete (be careful!)
        println!("═══ Querying Accounts to Delete ═══");
        // WARNING: This query selects ALL accounts created today. Adjust criteria safely.
        let soql = "SELECT Id, Name FROM Account WHERE CreatedDate = TODAY LIMIT 5";
        let mut stream = client.bulk().bulk_query::<Account>(soql).await?;

        let mut account_ids = Vec::new();
        while let Some(account) = stream.next().await? {
            println!("Found: {} ({})", account.id, account.name);
        ...
        println!("\nFound {} accounts to delete.", account_ids.len());
