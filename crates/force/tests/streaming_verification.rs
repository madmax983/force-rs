...
async fn test_streaming_csv_pipeline() {
    ...
    let stream = futures::stream::iter(vec![Ok::<bytes::Bytes, std::io::Error>(bytes::Bytes::from(data))]);
    ...
    // 1. Read Record 1
    let record1 = records.next().await.expect("Stream should have record 1").expect("Record 1 should be valid");
    assert_eq!(record1.id, "001");
    assert_eq!(record1.name, "Acme");

    // 2. Read Record 2
    let record2 = records.next().await.expect("Stream should have record 2").expect("Record 2 should be valid");
