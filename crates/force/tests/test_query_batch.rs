...
async fn test_query_batch_pagination_and_update() {
    ...
    let processor = QueryBatch::new(client.session(), "SELECT Id, Name FROM Account");
...
async fn test_query_batch_halt_on_error() {
    ...
    let processor = QueryBatch::new(client.session(), "SELECT Id, Name FROM Account").halt_on_error(true);
...
async fn test_query_batch_halt_on_error_false() {
    ...
    let processor = QueryBatch::new(client.session(), "SELECT Id, Name FROM Account").halt_on_error(false);
