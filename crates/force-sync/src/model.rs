...
fn hash_json_value(value: &Value, hasher: &mut blake3::Hasher, depth: u32) {
    use std::io::Write;
    if depth > 128 {
        // Prevent stack overflow on deeply nested JSON.
        // We drop the remaining structure rather than panicking,
        // to degrade safely for payloads that bypass initial parse limits.
        return;
    }
...
mod tests {
    ...
    #[test]
    fn payload_hash_terminates_gracefully_on_excessive_depth() {
        let mut current = json!({});
        // Construct a deeply nested JSON object that exceeds the 128 depth limit
        for _ in 0..200 {
            current = json!({ "nested": current });
        }

        // This should return a deterministic hash without panicking
        let hash = super::payload_hash(&current);
