...
fn hash_json_value(value: &Value, hasher: &mut blake3::Hasher) {
    ...
    match value {
        Value::Object(map) => {
            ...
            // ⚡ Bolt: Avoid allocating and sorting if the keys are already sorted (or if there are 0 or 1 elements).
            // Checking if sorted is O(N) which avoids O(N) allocation + O(N log N) sorting overhead.
