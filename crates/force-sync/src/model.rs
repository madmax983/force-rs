...
fn hash_json_value(value: &Value, hasher: &mut blake3::Hasher, depth: usize) {
    use std::io::Write;
    if depth > 128 {
        panic!("JSON structure exceeds maximum hashing depth of 128");
    }
    match value {
        Value::Object(map) => {
...
mod tests {
    ...
    #[test]
    #[should_panic(expected = "JSON structure exceeds maximum hashing depth of 128")]
    fn test_hash_json_value_depth() {
        use serde_json::{Map, Value};
        let mut current = Value::Null;
        for _ in 0..200 {
            let mut map = Map::new();
            map.insert("n".to_string(), current);
            current = Value::Object(map);
        }
        let _ = super::payload_hash(&current);
    }
