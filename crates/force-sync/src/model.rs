...
fn hash_json_value(value: &Value, hasher: &mut blake3::Hasher, depth: usize) {
    use std::io::Write;
    assert!(depth <= 128, "recursion limit exceeded");
    match value {
        Value::Object(map) => {
...
mod tests {
    ...
    #[test]
    fn test_havoc_payload_hash_dos() {
        ...
        let handler = builder
            .spawn(|| {
                ...
                for _ in 0..10_000 {
                    let mut map = Map::new();
                    map.insert("a".to_string(), current);
                    current = Value::Object(map);
                }

                let md = ManuallyDrop::new(current);
                let _hash = super::payload_hash(&md);
            })
            .unwrap_or_else(|e| panic!("Thread spawn failed: {e:?}"));

        let result = handler.join();
        assert!(result.is_err(), "Expected panic due to recursion limit");
        if let Err(e) = result {
            if let Some(s) = e.downcast_ref::<&str>() {
                assert_eq!(s, &"recursion limit exceeded");
            } else if let Some(s) = e.downcast_ref::<String>() {
                assert_eq!(s, "recursion limit exceeded");
            } else {
                panic!("Unknown panic type: {e:?}");
            }
        }
    }
}
