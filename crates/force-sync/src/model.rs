...
fn hash_json_value(value: &Value, hasher: &mut blake3::Hasher, depth: usize) {
    ...
    assert!(
        depth <= 128,
        "Payload hash depth limit exceeded (DoS prevention)"
    );
    match value {
        Value::Object(map) => {
...
mod havoc_dos_tests {
    ...
    #[test]
    #[should_panic(expected = "Payload hash depth limit exceeded (DoS prevention)")]
    fn test_payload_hash_dos_prevention() {
        let mut payload = json!({});
        // Use 150 depth to trigger the panic, but not overflow the stack
        for _ in 0..150 {
        ...
        // This will panic with our custom message instead of overflowing the stack!
        let _ = payload_hash(&payload);
    }
}
