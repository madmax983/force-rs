...
#[test]
fn test_exploit_stack_overflow_in_hash() {
    use serde_json::Value;
    // By creating a depth exactly around 150, we avoid `Drop` stack overflow
    // but we exceed our custom hasher limit of 128.
    let mut nested = Value::Null;
    for _ in 0..150 {
        nested = serde_json::json!([nested]);
    }
    // Should not crash the process with a stack overflow.
    let _hash = payload_hash(&nested);

    let mut less_nested = Value::Null;
    for _ in 0..128 {
        less_nested = serde_json::json!([less_nested]);
    }
    let _hash_less = payload_hash(&less_nested);

    // Because depth > 128 simply returns, deeper nesting will NOT hash the same as 128,
    // because the 150 array adds its own `[` and `]` characters up to depth 128, leaving the inner Null out.
    // The previous assert was incorrect. Let's just make sure it doesn't crash.
}
