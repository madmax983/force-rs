...
fn hash_json_value(value: &Value, hasher: &mut blake3::Hasher, depth: usize) {
    ...
    assert!(
        depth <= 128,
        "JSON depth limit exceeded during hashing to prevent stack overflow"
    );
    match value {
        Value::Object(map) => {
...
mod tests {
    ...
    #[test]
    #[should_panic(expected = "JSON depth limit exceeded")]
    fn havoc_deep_json_hash_stack_overflow() {
        let mut value = json!({});
        for _ in 0..1000 {
            value = json!({ "nested": value });
        }
        let _ = super::payload_hash(&value);
    }
