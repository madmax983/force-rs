...
/// ⚡ Bolt: Consuming `OutboxRow` by value eliminates `.clone()` allocations for `tenant`, `object_name`, and `external_id` on the hot path.
    ...
    for row in rows {
        ...
        match outbox_envelope(outbox_row) {
            ...
            Err(error) if row_content_error(&error) => {
                ...
                let _ = quarantine_row(client, &fallback_row, &error).await;
                processed = processed.saturating_add(1);
            }
