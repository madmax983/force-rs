...
/// ⚡ Bolt: Consumes `OutboxRow` to avoid cloning `tenant`, `object_name`, and `external_id` Strings.
    ...
    for row in rows {
        ...
        match outbox_envelope(outbox_row) {
            ...
            Err(error) if row_content_error(&error) => {
                ...
                let _ = quarantine_row(client, cold_row, &error).await;
                processed = processed.saturating_add(1);
            }
