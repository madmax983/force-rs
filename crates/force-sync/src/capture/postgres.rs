...
/// ⚡ Bolt: Consumes `OutboxRow` by value to eliminate `.clone()` heap allocations on the hot path.
    ...
    for row in rows {
        ...
        match outbox_envelope(outbox_row) {
            ...
            Err(error) if row_content_error(&error) => {
                let quarantine_outbox_row = OutboxRow {
                    outbox_id,
                    tenant: row.get("tenant"),
                    object_name: row.get("object_name"),
                    external_id: row.get("external_id"),
                ...
                let _ = quarantine_row(client, &quarantine_outbox_row, &error).await;
                processed = processed.saturating_add(1);
            }
