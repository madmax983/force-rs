...
fn outbox_operation(op: &str, tombstone: bool) -> crate::Result<ChangeOperation> {
    match (op, tombstone) {
...
fn outbox_source_cursor(raw: &str) -> crate::Result<SourceCursor> {
...
fn outbox_envelope(row: &OutboxRow) -> crate::Result<ChangeEnvelope> {
...
async fn quarantine_row<C>(client: &C, row: &OutboxRow, error: &ForceSyncError) -> crate::Result<()>
where
...
async fn capture_batch_in_tx<C>(client: &C, limit: i64, priority: i32) -> crate::Result<usize>
where
    C: GenericClient + Sync + ?Sized,
{
    if limit <= 0 {
...
pub async fn capture_batch(store: &PgStore, limit: i64, priority: i32) -> crate::Result<usize> {
    if limit <= 0 {
        return Ok(0);
    }
    store
