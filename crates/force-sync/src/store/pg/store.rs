...
impl PgStore {
    ...
    /// callback fails.
    pub async fn with_client<T, F, Fut>(&self, f: F) -> crate::Result<T>
    where
        F: FnOnce(Client) -> Fut,
    ...
    /// the callback fails.
    pub async fn with_transaction<T, F>(&self, f: F) -> crate::Result<T>
    where
        F: for<'a> FnOnce(&'a tokio_postgres::Transaction<'a>) -> BoxFuture<'a, crate::Result<T>>,
    {
