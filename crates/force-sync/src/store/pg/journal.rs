...
fn journal_values(envelope: &ChangeEnvelope) -> crate::Result<JournalValues<'_>> {
    let cursor = envelope
        .cursor()
...
async fn insert_journal<C>(client: &C, values: &JournalValues<'_>) -> crate::Result<i64>
where
...
impl PgStore {
    ...
    pub async fn append_journal(&self, envelope: &ChangeEnvelope) -> crate::Result<i64> {
