...
async fn detect_drift_query<C>(client: &C, limit: i64) -> crate::Result<Vec<DriftItem>>
where
...
async fn enqueue_repair_in_tx<C>(client: &C, journal_id: i64) -> crate::Result<i64>
where
    C: GenericClient + Sync + ?Sized,
{
...
async fn existing_repair_task_in_tx<C>(client: &C, journal_id: i64) -> crate::Result<Option<i64>>
where
    C: GenericClient + Sync + ?Sized,
{
...
pub async fn detect_drift(store: &PgStore, limit: i64) -> crate::Result<Vec<DriftItem>> {
...
pub async fn enqueue_repair(store: &PgStore, journal_id: i64) -> crate::Result<i64> {
    store
...
pub async fn run_reconcile_once(store: &PgStore, limit: i64) -> crate::Result<usize> {
    store
        .with_transaction(|tx| {
            async move {
