...
impl<A: Authenticator> SyncEngine<A> {
    ...
    pub async fn run_capture_postgres_once(&self) -> crate::Result<usize> {
    ...
    pub async fn run_apply_once(&self) -> crate::Result<usize> {
        let leased = self
            .store
    ...
    pub async fn run_reconcile_once(&self) -> crate::Result<usize> {
    ...
    async fn load_apply_task_context(
        &self,
        task_id: i64,
    ) -> crate::Result<Option<ApplyTaskContext>> {
...
impl<A: Authenticator> SyncEngineBuilder<A> {
    ...
    pub fn build(self) -> crate::Result<SyncEngine<A>> {
        let store = self
            .postgres
...
fn build_apply_task_context(row: &tokio_postgres::Row) -> crate::Result<ApplyTaskContext> {
...
fn local_projection_salesforce_id(
    existing_link: Option<&crate::store::pg::SyncLink>,
    envelope: &ChangeEnvelope,
) -> crate::Result<Option<force::types::SalesforceId>> {
...
fn parse_source_system(value: &str) -> crate::Result<SourceSystem> {
    match value {
...
fn parse_change_operation(value: &str) -> crate::Result<ChangeOperation> {
    match value {
...
fn parse_source_cursor(value: &str) -> crate::Result<SourceCursor> {
