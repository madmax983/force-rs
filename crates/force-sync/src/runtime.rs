...
impl<A: Authenticator> SyncEngine<A> {
    ...
    async fn apply_rest_task(
        ...
        match envelope.operation() {
            ChangeOperation::Upsert => {
                self.apply_rest_upsert(task, envelope, payload, existing_link, object)
                    .await
            }
            ChangeOperation::Delete => self.apply_rest_delete(task, envelope, existing_link).await,
        }
    }

    async fn apply_rest_upsert(
        &self,
        task: &LeasedTask,
        envelope: &ChangeEnvelope,
        payload: &Value,
