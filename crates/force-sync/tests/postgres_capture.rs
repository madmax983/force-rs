...
async fn dead_letter_count(pool: &deadpool_postgres::Pool) -> force_sync::error::Result<i64> {
    let client = pool.get().await?;
    let row = client
...
async fn error_message_for_external_id(
    pool: &deadpool_postgres::Pool,
    external_id: &str,
) -> force_sync::error::Result<Option<String>> {
    let client = pool.get().await?;
    let row = client
        .query_opt(
...
#[tokio::test]
#[ignore = "requires FORCE_SYNC_TEST_DATABASE_URL"]
async fn duplicate_source_cursors_do_not_enqueue_duplicate_tasks() -> force_sync::error::Result<()>
{
    let pool = support::postgres::test_pool();
    support::postgres::reset_schema(&pool).await?;
    force_sync::migrate(&pool).await?;
...
#[tokio::test]
#[ignore = "requires FORCE_SYNC_TEST_DATABASE_URL"]
async fn contradictory_op_and_tombstone_rows_are_rejected_safely() -> force_sync::error::Result<()>
{
    let pool = support::postgres::test_pool();
    support::postgres::reset_schema(&pool).await?;
    force_sync::migrate(&pool).await?;
...
#[tokio::test]
#[ignore = "requires FORCE_SYNC_TEST_DATABASE_URL"]
async fn invalid_outbox_row_is_dead_lettered_and_marked_processed() -> force_sync::error::Result<()>
{
    let pool = support::postgres::test_pool();
    support::postgres::reset_schema(&pool).await?;
    force_sync::migrate(&pool).await?;
...
#[tokio::test]
#[ignore = "requires FORCE_SYNC_TEST_DATABASE_URL"]
async fn already_encoded_cursor_is_rejected_safely() -> force_sync::error::Result<()> {
    let pool = support::postgres::test_pool();
    support::postgres::reset_schema(&pool).await?;
    force_sync::migrate(&pool).await?;
