...
mod tests {
    ...
    async fn test_lease_duration_panic() {
        ...
        let result = store.lease_ready_tasks("worker", 10, Duration::from_secs(i64::MAX as u64)).await;

        // Now it should return a ForceSyncError::InvalidLeaseDuration
        assert!(matches!(result, Err(force_sync::ForceSyncError::InvalidLeaseDuration)));
    }
}
