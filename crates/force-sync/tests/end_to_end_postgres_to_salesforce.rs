...
async fn run_capture_and_apply_once_converges_one_postgres_record() -> Result<(), force_sync::ForceSyncError> {
...
#[tokio::test]
#[ignore = "requires FORCE_SYNC_TEST_DATABASE_URL"]
async fn replayed_postgres_change_is_not_reapplied_to_salesforce() -> Result<(), force_sync::ForceSyncError> {
    let mock_server = MockServer::start().await;
    let client = test_client(&mock_server).await;
