...
async fn cdc_event_creates_journal_task_and_checkpoint() -> force_sync::Result<()> {
...
#[tokio::test]
#[ignore = "requires FORCE_SYNC_TEST_DATABASE_URL"]
async fn duplicate_cdc_event_is_deduped_by_replay_id() -> force_sync::Result<()> {
    let pool = support::postgres::test_pool();
    support::postgres::reset_schema(&pool).await?;
    force_sync::migrate(&pool).await?;
    let store = PgStore::new(pool.clone());
...
#[tokio::test]
#[ignore = "requires FORCE_SYNC_TEST_DATABASE_URL"]
async fn restart_uses_the_stored_replay_cursor() -> force_sync::Result<()> {
    let pool = support::postgres::test_pool();
    support::postgres::reset_schema(&pool).await?;
    force_sync::migrate(&pool).await?;
    let store = PgStore::new(pool.clone());
